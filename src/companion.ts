...
export const getProblemGroup = (problem: Problem) => {
    const words = words_in_text(problem.group);
    if (words === null) {
        return `${problem.group.replace(/\W+/g, '_')}`;
    } else {
        return `${words.join('_')}`;
    }
};
...
const handleNewProblem = async (problem: Problem) => {
    ...
    const group = getProblemGroup(problem);
    const srcPath = path.join(folder, group, problemFileName);
    ...
    createFolderIfNotExists(path.dirname(srcPath));
